...
public class BlogController {
    ...
    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "Create blog post (JSON)", description = "Create blog without image")
    ...
    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "Create blog post (multipart)", description = "Create blog with image upload")
    public ResponseEntity<ResponseData<BlogResponse>> createBlogWithImage(
            @RequestPart("request") @Valid BlogRequest request,
            @RequestPart(value = "image", required = false) MultipartFile image) throws IOException {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new ResponseData<>(HttpStatus.CREATED.value(), "Blog created",
    ...
    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "Update blog post (JSON)", description = "Update blog without image")
    ...
    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "Update blog post (multipart)", description = "Update blog with image upload")
    public ResponseEntity<ResponseData<BlogResponse>> updateBlogWithImage(@PathVariable Long id,
            @RequestPart("request") @Valid BlogRequest request,
            @RequestPart(value = "image", required = false) MultipartFile image) throws IOException {
        return ResponseEntity
                .ok(new ResponseData<>(HttpStatus.OK.value(), "Blog updated",
                        blogService.updateBlog(id, request, image)));
    }
